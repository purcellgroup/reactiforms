...
export function FormInstance(config) {
  const writeToNextInputs = () => {};
  const registerInput = () => {};
  const unregisterInput = () => {};
  const updateMapInput = () => {};
  const broadcastChange = () => {};

  //! create a buffer function that generates a public object
  //! instantiate this instance in that function

  const returnPublicObject = () => ({
    formId: GEN_FORM_ID(),
...
export function createForm(config) {
  ...
  form.Input = _createInput(form);
  form.FormProvider = _createForm(form);
  form.useInput = _useInput({
    getInput: form.getInput,
    subscribeToInput: subscribeToInput(form),
    unsubscribeToInput: unsubscribeToInput(form),
  });
  return form;
}
...
// !! param is a map
export const resetFormValues = (inputs: InputMap) => {
...
export const getFormValues = (inputs: InputMap) =>
  Array.from(inputs).reduce(
    (acc: Record<string, string>, [id, input]): FormValues => {
      acc[id] = input.value;
      return acc;
    },
    {}
...
export const getFormInputs = (inputs: InputMap) => {
  if (inputs.size) {
...
export const validateForm = (inputs: InputMap) =>
